...
frappe.ui.form.on("Work Assigned", {
  main_parts: function (frm, cdt, cdn) {
    ...
    frappe.meta.get_docfield("Work Assigned", "task", frm.doc.name).get_query =
      function () {
        return {
          filters: {
...
frappe.ui.form.on("Complain Table", {
  budn: function (frm, cdt, cdn) {
    var row = locals[cdt][cdn];
    console.log("we are here");
    frappe.meta.get_docfield("Complain Table", "act", frm.doc.name).get_query =
      function () {
        return {
          filters: {
    ...
    frappe.call({
      ...
      callback: function (response) {
        if (response.message) {
          ...
          frm.fields_dict["items"].grid.get_field("location").get_query =
...
frappe.ui.form.on("Maintenance Work order", {
  group: function (frm, cdt, cdn) {
    ...
    frm.fields_dict["complain_detail"].grid.get_field("name1").get_query =
      function (doc, cdt, cdn) {
        return {
          filters: {
          ...
          frappe.call({
            ...
            callback: async function(rsp) {
              if (rsp && rsp.message) {
                ...
                frm.clear_table('work_done_completed_by_inspector'); // Clear existing rows
                // Adding unique tasks to the table
                Object.values(uniqueTasks).forEach(task => {
                  var table = frm.add_child('work_done_completed_by_inspector');
                  table.main_parts = task.equipment_specific_type;
                  table.task = task.task;
                  table.duration = task.duration;
                });
                frm.refresh_field('work_done_completed_by_inspector'); // Refresh the table
              }
            },
          });

          // Update replaced parts and labor cost summary
          frappe.call({
            method: 'frappe.client.get_list',
            args: {
              doctype: 'Replaced parts',
             
              fields: ['*'],
            },
            callback: async function(rsp) {
              if (rsp && rsp.message) {
                var replacedParts = rsp.message;
                console.log('replaced parts', replacedParts);
                var totalCost = 0;
                ...
                var table = frm.add_child('replaced_part_and_labor_cost_summary');
                table.description = 'Replaced Parts';
  console.log(table + "table replaced paert");
                table.amount = totalCost;
                frm.refresh_field('replaced_part_and_labor_cost_summary'); // Refresh the table
              }
            },
          });
        }
...
frappe.ui.form.on("Maintenance Work order", {
  before_submit: function (frm) {
    ...
    frappe.call({
      ...
      callback: function (r) {
        frappe.msgprint(
          `Vehicle or Machinery (${frm.doc.serial_or_plate_no}) set to Non-Operable.`
        );
  ...
  on_trash: function (frm) {
    ...
    frappe.call({
      ...
      callback: function (r) {
        frappe.msgprint(
          `Vehicle or Machinery (${frm.doc.serial_or_plate_no}) set to Operable.`
        );
        console.log("message", r);
      },
    });
