...
def update_employee(employee, details, date=None, date_ec=None, cancel=False):
	...
	for item in details:
		...
		if item.fieldname in ["department", "designation", "work_place", "project"]:
			internal_work_history[item.fieldname] = item.new
	if internal_work_history and not cancel:
		internal_work_history["from_date"] = date
		internal_work_history["from_date_ec"] = date_ec
...
def get_employee_fields_label():
	...
	for df in frappe.get_meta("Employee").get("fields"):
		if df.fieldname in ["salutation", "user_id", "employee_number", "employment_type",
			"holiday_list","department", "designation", "grade",
			"notice_number_of_days", "reports_to", "leave_policy", "company_email","salary", "word_salary","office","maintenance_office","projects"]:
