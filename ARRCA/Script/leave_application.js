...
var total_free_days=0;
 ...
 // Fetch holidays and update total leave days
        get_holidays(frm.doc.employee, frm.doc.half_end_date_gc, frm.doc.half_start_date_gc, function(holidays) {
            ...
            if (holidays !== null && holidays !== undefined) {
                ...
                console.log("Total leave days after deducting holidays", holidays_value );
            }
        });
         //check holidays
...
total_value = (frm.doc.total_leave_day_hi|| 0) + (total_free_days );
      console.log("frm.doc.total_leave_day_hi total ", frm.doc.total_leave_day_hi);
...
if(employee && from_date && to_date){
    frappe.call({
        method: 'erpnext.hr.doctype.leave_application.leave_application.get_holidays2_days',
        args: {
            "employee": employee,
            "from_date": from_date,
            "to_date": to_date,
