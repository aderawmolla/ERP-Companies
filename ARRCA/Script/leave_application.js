frappe.ui.form.on("Leave Application", {
    ...
    half_day_request:function(frm,cdt,cdn){
      var days=  getDaysBetweenDates(frm,frm.doc.requsted_from_date_gc,frm.doc.requested_to_date_gc)
    ...
    total_leaves_allocated: function (frm) {
      if (frm.doc.__islocal) {
        // Check if the document is not saved (new document)
  ...
  function getDaysBetweenDates(frm,from_date, to_date) {
   
      const startDate = new Date(from_date);
      const endDate = new Date(to_date);
      // Calculate the difference in milliseconds
      const differenceInMilliseconds = endDate - startDate;
     if(frm.doc.half_day_request==1){
        return (Math.ceil(differenceInMilliseconds / (1000 * 60 * 60 * 24))+1)/2;
