...
frappe.ui.form.on("Employee", {
    date_of_birth_ec: function (frm, cdt, cdn) {
        ...
        frm.doc.date_of_birth = convertDateTOGC(frm.doc.date_of_birth_ec.toString());;
...
function calculateDay(frm, cdt, cdn) {
    ...
    var finalDiff = year_diff + ' years ' + month_diff + ' months ' + day_diff + ' days ';
...
function calculateDayInternal(frm, cdt, cdn) {
    ...
    table.map((item, index) => {
        if (item.from_date_ec && item.to_date_ec) {
            if (item.designation == "ትምህርት ክፍል") {
                ...
                var finalll = year_diffs + ' years ' + month_diffs + ' months ' + day_diffs + ' days ';
    ...
    var finalDiffs = total_year_diff + ' years ' + total_month_diff + ' months ' + total_day_diff + ' days ';
