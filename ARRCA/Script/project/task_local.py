...
def get_children(doctype, parent, task=None, project=None, is_root=False):
    ...
    # Sort the tasks with a custom key
    def alphanumeric_key(task):
        # Split subject into parts that are either digits or non-digits
        return [int(part) if part.isdigit() else part for part in re.split('(\d+)', task['title'])]

    # Sort the tasks using the alphanumeric key
    tasks.sort(key=alphanumeric_key)
