...
cur_frm.add_fetch('city_code', 'city_name', 'city_name');
frappe.ui.form.on('Delivery Note', {
    loading_advice: function (frm, cdt, cdn) {
        frappe.call({
            method: "frappe.client.get_list",
            args: {
                doctype: 'Loading Advice Detail',
                filters: { "parent": frm.doc.loading_advice },
                fields: ['*'],

            },
            callback: function (response) {
                console.log("the response is ",response)
            var response=response.message[0];
             frm.doc.items=[];   
            var child= frm.add_child("items") 
            child.item_code=response.item_code
            child.uom=response.uom
            child.item_name=response.item_name
            child.description=response.description
            child.qty=response.qtybulk_in_ton
            child.rate=response.rate
            child.amount=response.rate*response.response.qtybulk_in_ton
            child.name1=response.name1
            child.against_sales_order=response.parent1
            refresh_field("items")
    ...
    frappe.call({
        ...
        args: {
            doctype: 'Loading Advice',
            filters: { "name": frm.doc.loading_advice },
            fieldname: ['*'],
        ...
        callback: function (response) {
          console.log("the response is ",response)
           var response=response.message;
            frm.set_value("sales_order",response.order_no)
            frm.set_value("customer",response.customer)
            frm.refresh_field("sales_order")
            frm.refresh_field("ustomer")
...
//when item_category selected(linked field ) it assigns the item_sub_category when the item_sub_category item_category is matched
    refresh: function(frm, cdt, cdn) {
     frm.set_query("loading_advice", function() {
        return {
            "filters": {
