...
erpnext.selling.SalesOrderController = erpnext.selling.SellingController.extend({
		...
		if (doc.docstatus==1) {
			...
			if(doc.status !== 'Closed') {
				if(doc.status !== 'On Hold') {
                    ...
                    this.frm.add_custom_button(__('Shipment Coupon'), () => this.create_shipment_coupon(), __('Create'));
  ...
  create_shipment_coupon() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_order.sales_order.create_shipment_coupon",
			frm: this.frm
		})
	},
	make_work_order() {
