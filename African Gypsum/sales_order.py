	...
	def validate_delivery_date(self):
		if self.order_type == 'Sales' and not self.skip_delivery_note:
			...
			if self.delivery_date:
				...
				pass
				# self.delivery_date = frappe.datetime.nowdate()
...
def make_delivery_note_of_loading(source_name, target_doc=None, skip_item_mapping=False):
	def set_missing_values(source, target):
		...
		target.run_method("set_po_nos")
		target.run_method("calculate_taxes_and_totals")
		target.sales_order=source.order_no
		# if source.company_address:
		# 	target.update({'company_address': source.company_address})
		# else:
		# 	# set company address
...
@frappe.whitelist()
def create_shipment_coupon(source_name,target_doc=None):
	...
	def set_missing_values(source, target):
		...
		target.fs_no=source.external_doument_no
		# target.updated_from=source_name
