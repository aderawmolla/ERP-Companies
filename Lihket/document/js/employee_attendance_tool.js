...
erpnext.EmployeeSelector = Class.extend({
	...
	make: function(frm, employee) {
		...
		var employee_toolbar = $('<div class="col-sm-12 top-toolbar">\
            ...
            var mark_employee_toolbar = $('<div class="col-sm-12 bottom-toolbar">\
                ...
                <button class="btn btn-default btn-mark-on-field btn-xs"></button></div>');
    ...
    //   I code  untill my mother become rich a

    mark_employee_toolbar.find(".btn-mark-on-field")
    .html(__('Mark On Field'))
    .on("click", function() {
        var employee_on_field = [];
        $(me.wrapper).find('input[type="checkbox"]').each(function(i, check) {
            if($(check).is(":checked")) {
                employee_on_field.push(employee[i]);
            }
        });
        frappe.call({
            method: "erpnext.hr.doctype.employee_attendance_tool.employee_attendance_tool.mark_employee_attendance",
            args:{
                "employee_list":employee_on_field,
                "status":"On Field",
                "date":frm.doc.date,
                "attendance_date_ec":frm.doc.attendance_date_ec,
                "company":frm.doc.company
