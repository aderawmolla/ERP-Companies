frappe.ui.form.on("Leave Application", {
  ...
  half_start_date_ec: function (frm, cdt, cdn) {
    if (frm.doc.half_start_date_ec) {
      ...
      halfDay(frm);
      setDateConverted(frm, finalgc, "from_date");
    }
  },
  half_end_date_ec: function (frm, cdt, cdn) {
    if (frm.doc.half_end_date_ec) {
      ...
      //frm.refresh_field("total_leave_days");
      halfDay(frm);
      setDateConverted(frm, finalgc, "to_date");
    }
  },
});

function setReturnDate(frm) {
    if (!frm.doc.to_date) {
        frappe.msgprint(__("Please set a valid 'to_date'."));
        return;
    }
   
    // Parse the `to_date` field
    let to_date = new Date(frm.doc.to_date);

    // Extract day of the week, month, date, and year
    ...
    let day = daysOfWeek[to_date.getDay()];
    let month = months[to_date.getMonth()];
    let date = to_date.getDate();
    let year = to_date.getFullYear();

    // Determine if it's Morning or Afternoon
    let hours = to_date.getHours();
    let period = hours < 12 ? "Morning" : "Afternoon";

    // Format the final date string
    let formattedDate = `${day}, ${month} ${date}, ${year} (${period})`;

    // Set the formatted string to the `return_date` field
    frm.set_value("return_date", formattedDate);
}
...
frappe.ui.form.on("Leave Application", {
  total_leave_days: function (frm, cdt, cdn) {
    
  },
  total_days: function (frm, cdt, cdn) {
    if (frm.doc.total_days) {
        var startDate;
        // Check if the "from_date" field is set
        if (frm.doc.from_date) {
          startDate = new Date(frm.doc.from_date);
        }
        // Check if "half_start_date_gc" overrides the "from_date"
        if (!frm.doc.from_date && frm.doc.half_start_date_gc) {
          startDate = new Date(frm.doc.half_start_date_gc);
        }
    
        // Ensure startDate is valid before proceeding
        if (startDate) {
          // Add total_leave_days to the startDate
          if (frm.doc.total_days) {
            console.log("Start date here is ",startDate)
            startDate.setDate(startDate.getDate() + parseInt(frm.doc.total_days));
            // Format the new date (optional)
            let formattedDate = startDate.toISOString().split("T")[0];
            
            // Display the calculated date in the console or another field (optional)
            console.log("New Date after adding leave days:", formattedDate);
    
            // You can set the calculated date to another field on the form, e.g., "end_date"
            frm.set_value("to_date", formattedDate);
            frm.refresh_field("to_date");
          }
        } else {
          frappe.msgprint(__("Please set a valid start date."));
        }
    }
  },
});

function setDateConverted(frm, date, field) {
  var dateObject = new Date(date);
  // Format the date as a string in a desired format
  var formattedDate = dateObject.toISOString().slice(0, 10); // YYYY-MM-DD
  if (!frm.doc[field]) {
    frm.set_value(field, formattedDate);
  }
