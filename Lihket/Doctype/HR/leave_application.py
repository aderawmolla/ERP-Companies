			...
			if leave_type.applicable_after > 0:
				...
				if number_of_days >= 0:
					...
					if not frappe.db.get_value("Leave Type", self.leave_type, "include_holiday"):
						holidays = get_holidays(date_of_joining, self.from_date)
...
def get_number_of_leave_days(employee, leave_type, from_date, to_date, half_day = None, half_day_date = None):
	...
	number_of_days = flt(number_of_days) - flt(get_holidays(from_date, to_date))
...
@frappe.whitelist()
def get_holidays2_days(employee, from_date, to_date):
	'''get holidays between two dates for the given employee'''
	holiday_list = get_holiday_list_for_employee(employee)

	holidays = frappe.db.sql("""
		SELECT holiday_date, DAYOFWEEK(holiday_date) AS day_of_week
		FROM `tabHoliday` h1, `tabHoliday List` h2
		WHERE h1.parent = h2.name
		AND h1.holiday_date BETWEEN %s AND %s
		AND h2.name = %s
	""", (from_date, to_date, holiday_list), as_dict=True)

	total_holidays = 0
	
	for holiday in holidays:
		holiday_date = holiday.get('holiday_date')
		day_of_week = holiday.get('day_of_week')

		if day_of_week == 7:  # Saturday
			total_holidays += 0.5
		elif day_of_week == 1:  # Sunday
			total_holidays += 0
		else:
			total_holidays += 1
...
def get_holidays(from_date, to_date):
	try:
		...
		formatted_from_date = "-".join(from_date.split("-")[::-1])
		formatted_to_date = "-".join(to_date.split("-")[::-1])
		
		# Log the converted dates for debugging
		
		# frappe.logger().debug("Fetching holidays from {} to {}".format(formatted_from_date, formatted_to_date))

		# Fetch holidays from the database
		holidays = frappe.db.sql("""
			SELECT COUNT(DISTINCT h1.holiday_date)
			FROM `tabHoliday` h1
			JOIN `tabHoliday List` h2 ON h1.parent = h2.name
			WHERE h1.holiday_date BETWEEN %s AND %s
		""", (from_date, to_date))
		
		# Log the retrieved data for debugging

		# frappe.logger().debug("Holidays fetched: {}".format(holidays))
		...
		frappe.throw(_("An error occurred while fetching holidays: {0}").format(str(e)))
...
#             SUM(
#                 CASE 
#                     WHEN h2.description = 'Saturday' THEN 0.5 
#                     ELSE 1 
#                 END
#             ) AS total_holidays 
#         FROM 
#             `tabHoliday` h1 
#         WHERE 
#             h1.holiday_date BETWEEN %s AND %s 
#             AND h1.parent = %s
#     """, (from_date, to_date, holiday_list))[0][0]
